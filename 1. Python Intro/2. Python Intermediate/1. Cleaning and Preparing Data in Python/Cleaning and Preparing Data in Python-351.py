...
# moma <- moma %>% 
#  mutate(Nationality = str_replace_all(Nationality,"[^A-Za-z0-9 ]","")) %>% 
# mutate(Gender = str_replace_all(Gender,"[^A-Za-z0-9]",""))
...
for row in moma:
    birth_date = row[3]
    death_date = row[4]
    row[3] = clean_and_convert(birth_date)
    row[4] = clean_and_convert(death_date)
