...
def extract_last_word(element):
    return str(element).split()[-1] #dont forget the return
...
merged['Currency Vectorized'] = merged['CurrencyUnit'].str.split().str.get(-1) # -1 indicates select last word
...
first_two_year = years['First_Year'].str[0:2] #[] square brackets for slicing/indexing first two characters of a string
...
pv_incomes = merged.pivot_table(index = 'IncomeGroup', values = 'Happiness Score') #default function is mean

pv_incomes.plot(kind = 'bar', rot = 30, ylim = (0,10)) #panda method
plt.show()
